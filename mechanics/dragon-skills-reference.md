# Dragon Skills Reference — From Dragon Pit Screens

*Extracted from IMG_2663-2748, IMG_2897-2922 (Dragon Pit management screens, 2026-06-16 to 2026-06-19)*

---

## Active Skills (Combat Abilities)

### Arulix (Champion Whelp) — Gleaming Spiral
- **Every round:** 25% chance to reduce Fire Damage dealt by *all enemies that deal Fire damage* by 15% for 1 round
- **On rounds 1, 2, 3, 5, 8:** Deals **Tactical Damage** to *all enemies that deal Physical Damage* (does not affect Basic-Attack-only enemies), with **+45% Damage Rate**
- **At 6+ Stars:** On rounds 1, 2, 3, 5, 8: Deals **Physical Damage** to 2 enemies in any combat lane, **+20% Damage Rate**. Boosted 1.5x (+30%) on round 5, 2x (+40%) on round 8

---

### Nyrena (Champion Hatchling) — Undermine
- **Tactical Damage** to target at **80% damage rate** (scales with attacker's Instinct, reduced by target's Intelligence)
- **Physical Damage Dealt Modifier:** Applies debuff reducing target's **Physical Damage received by 10%**

---

### Shimmer (Sentinel Whelp) — Unbreakable Loyalty
- **Tactical Damage** to target at **+50% damage rate** (scales with attacker's Instinct, reduced by target's Intelligence)
- **Strength Modifier:** Debuff lowering target's **Strength by 18%** (boosted by attacker's Instinct)
- **Initiative Modifier:** Debuff lowering target's **Initiative** (details partially cut off, boosted by Instinct)

---

### Arrax (Warrior Hatchling) — Sudden Strike
- **On rounds 2, 4, 6, 8:** 25% chance to apply **Weakened** debuff (reduces target's damage dealt) to 1 enemy adjacent to primary target. **Chance doubles to 50% if target already has Bleed**
- **On rounds 2, 4, 5, 6, 8:** Deals **Physical Damage** to 2 enemies adjacent to primary target, at **+40% damage rate**

---

### Shadowsong (Hunter Whelp, Lv 28, 2★) — Breath of Fire (Command) + Ensare (Habit)

**Breath of Fire (Command — Active)**
- **Triggers:** Rounds 2, 5, 8
- **Base (3★):** Deal **Fire Damage** to 2 Enemies within adjacency (Damage Rate: **+100%**). Damage increased **1.5x** if target afflicted with **Panic** (Damage Rate: **+150%**)
  - Scales with attacker's **Intelligence**, mitigated by target's **Initiative**
- **At 10★:** Fire to 1 Enemy in any lane (DR: **+60%**) + 40% chance Burn 2 rounds. Then Fire to different Enemy any lane (DR: **+30%**) + 20% chance Burn 2 rounds
- **Alternate 10★ description:** Fire to 2 Enemies in adjacent positions (DR: +60%), 1.5x vs Panic (+150%)

**Ensare (Habit — Passive)**
- **Trigger:** Round 1
- **Effect:** Reduce **Instinct** and **Initiative** by **-18%** (enhanced by Intelligence) of 2 Enemies within adjacency for 3 rounds
- **Upgrade Table:**
  | Level | Instinct/Initiative Reduction | Ability Power |
  |-------|-------------------------------|---------------|
  | 1 (current) | -18% | 340 |
  | 2 | -21.6% | 740 |
  | 3 | -25.2% | 1,200 |
  | 4 | -30.6% | 1,800 |
  | 5 | -36% | 2,500 |
- **Upgrade Cost (L1→L2):** 100 Purple Cores + 100 Green Cores + 50 Gold
- **Note:** Habit Upgrades, Breedmarks, and Rarity Cores are permanent and carry over between Reins

**Shadowsong Stats (Lv 28, 2★, Hunter Whelp)**
- **Attack:** 98.6
- **Defense:** 87.0
- **Speed:** 115.2
- **Flight:** 88.3
- **Class:** Average
- **Energy:** 250 / 100
- **Level Progress:** 69 / 100
- **Stars:** 2★ (2 unlocked abilities, 4 locked slots)
- **Resources:** 3,864.0 Food / 13,540 Gold / Player Lv 25

---

### Arulix (Champion Whelp) — Gleaming Spiral
- **Every round:** 25% chance to reduce Fire Damage dealt by *all enemies that deal Fire damage* by 15% for 1 round
- **On rounds 1, 2, 3, 5, 8:** Deals **Tactical Damage** to *all enemies that deal Physical Damage* (does not affect Basic-Attack-only enemies), with **+45% Damage Rate**
- **At 6+ Stars:** On rounds 1, 2, 3, 5, 8: Deals **Physical Damage** to 2 enemies in any combat lane, **+20% Damage Rate**. Boosted 1.5x (+30%) on round 5, 2x (+40%) on round 8

---

### Nyrena (Champion Hatchling) — Undermine
- **Tactical Damage** to target at **80% damage rate** (scales with attacker's Instinct, reduced by target's Intelligence)
- **Physical Damage Dealt Modifier:** Applies debuff reducing target's **Physical Damage received by 10%**

---

### Shimmer (Sentinel Whelp) — Unbreakable Loyalty
- **Tactical Damage** to target at **+50% damage rate** (scales with attacker's Instinct, reduced by target's Intelligence)
- **Strength Modifier:** Debuff lowering target's **Strength by 18%** (boosted by attacker's Instinct)
- **Initiative Modifier:** Debuff lowering target's **Initiative** (details partially cut off, boosted by Instinct)

---

### Arrax (Warrior Hatchling) — Sudden Strike
- **On rounds 2, 4, 6, 8:** 25% chance to apply **Weakened** debuff (reduces target's damage dealt) to 1 enemy adjacent to primary target. **Chance doubles to 50% if target already has Bleed**
- **On rounds 2, 4, 5, 6, 8:** Deals **Physical Damage** to 2 enemies adjacent to primary target, at **+40% damage rate**

---

### Daemoros (Warrior Hatchling, Lv 29, 2★) — Shadowflame (Command)

**Shadowflame (Active)**
- **Trigger:** Odd-numbered rounds (1, 3, 5, 7, 9...)
- **Effect:** Deal **Physical Damage** to 1 adjacent enemy (Damage Rate: **+125%**)
  - Scales with attacker's **Strength**, mitigated by target's **Instinct**
- **Secondary:** 20% chance to apply **Burn** (Fire DoT) for 2 rounds (+20% Damage Rate over 2 rounds)
- **Daemoros Stats (Lv 29, 2★, Warrior Hatchling):**
  - **Strength:** 4,166.0
  - **Health:** 12,950
  - **HP:** 12,020 / 108,261
  - **Ability Level:** 12 / 40
  - **Level:** 29, 2★

---

### Vaeldra (Warrior Whelp, Lv 29, 3★) — Lure (Command) + Dragon's Valor (Habit)

**Lure (Active)**
- **Taunt Effect:** Each Round: 25% chance to **Taunt** 3 Enemies in any lane for 2 rounds
- **Damage Effect:** Odd-numbered rounds: Deal **Physical Damage** to 2 adjacent enemies (Damage Rate: **+45%**)
  - Scales with attacker's **Strength**, mitigated by target's **Instinct**
- **Vaeldra Stats (Lv 29, 3★, Warrior Whelp):**
  - **Level:** 29, 3★
  - **HP:** 32,614 / 97,901
  - **Level Progress:** 61 / 100
  - **Stars:** 3★
  - **Class:** Warrior Whelp

**Dragon's Valor (Habit — Passive)**
- **Trigger:** Start of Combat
- **Effect:** Reduce Damage Received by **-5%**, increase **Strength** by **+8.5%**
- **Upgrade Table:**
  | Level | Damage Received | Strength | Power |
  |-------|-----------------|----------|-------|
  | 1 (current) | -5% | 8.5% | 340 |
  | 2 | -6% | 10.2% | 740 |
  | 3 | -7% | 11.9% | 1,200 |
  | 4 | -8.5% | 14.45% | 1,800 |
  | 5 | -10% | 17% | 2,500 |
- **Upgrade Cost (L1→L2):** 100 Purple Cores + 100 Green Cores + 176 Gold

**Vaeldra Stats (Lv 29, 3★, Warrior Whelp):**
- **HP:** 32,614 / 97,901
- **Level Progress:** 61 / 100
- **Stars:** 3★
- **Class:** Warrior Whelp
- **Stats (from stats panel):**
  - **Head/Attack:** 115.2
  - **Rings/Defense:** 108.8
  - **Eye/Speed:** 87.0
  - **Wing/Flight:** 94.7
  - **Flag/Tier:** Average
  - **Lightning/Stamina:** 100 / 100
  - **Level Progress:** 61 / 100

---

### Vaeldra (Warrior Whelp, Lv 27, 1★) — Dragon's Valor (Habit) — OLD VERSION
*Note: Earlier screenshots show Vaeldra at Lv 27, 1★ with same Dragon's Valor habit*

---

### Vaeldra (Warrior Whelp, Lv 27, 1★) — Lure (Active) — OLD VERSION
*Note: Earlier screenshots show Vaeldra with Lure ability at lower level*

---

## Dragon Roster (from Dragon Pit screens)

### High-Level Dragons (Lv 25-29, 3-4★)
| Dragon | Level | Stars | Class | Army Assignment (Updated) |
|--------|-------|-------|-------|---------------------------|
| Vhagar | — | — | Sentinel | Army 1 Van |
| Jagadrix | 28-29 | 3-4★ | Warrior | Army 1 Left |
| Vesper | 28-29 | 3★ | Sentinel | Army 2 Van |
| Antares | 29 | 1-4★ | Champion | Army 2 Right |
| Bevlorin | 27-28 | 2★ | Warrior | Army 1 Right |
| Tashix | 27-28 | 2★ | Hunter | Army 2 Left |
| Rhysarion | 24-27 | 2★ | Champion | Army 4 Left |
| Shimmer | 25-28 | 2★ | Sentinel | Army 3 Right |
| Feskar | 28 | 1★ | Warrior | Army 3 Left |
| Arrax | 27 | 2★ | Warrior | Army 3 Van |
| Solstryker | 26 | 2★ | Champion | Army 4 Van |
| Velar | 23 | 1★ | Champion | Army 4 Right |
| Nyrena | 25-27 | 2★ | Champion | Army 5 Van |
| Dawnseeker | 29 | 1★ | Sentinel | Army 5 Right |
| Seasmoke | 25 | 2★ | — | Army 5 Left |
| **Daemoros** | **29** | **2★** | **Warrior** | **Army 3 Left** |
| **Vaeldra** | **29** | **3★** | **Warrior** | **Army 3 Right** |
| **Shadowsong** | **28** | **2★** | **Hunter** | **Army 5 Van** |

### Other Dragons Seen
| Dragon | Level | Stars | Class | Notes |
|--------|-------|-------|-------|-------|
| Shadowsong | 28 | 2★ | Hunter | Dragon Pit roster, Lv28 2★ |
| Shadowrend | 29 | 4★ | Warrior | Dragon Pit roster |
| Thunderstrike | 29 | 4★ | — | Dragon Pit roster |
| Daemoros | 29 | 2★ | Warrior | Dragon Pit roster, Lv29 2★ |
| Vaeldra | 29 | 3★ | Warrior | Dragon Pit roster, Lv29 3★ |
| Arulix | 27 | 2★ | Champion | Dragon Pit roster |
| Caraxes | 25 | 3★ | — | Dragon Pit roster |
| Kalspire | 25 | 3★ | — | Dragon Pit roster |
| Crimson | 25 | 3★ | — | Dragon Pit roster |
| Venator | — | — | — | Dragon Pit roster (egg visible, new) |
| Kalspire | 25 | — | — | Dragon Pit roster (egg) |
| Caraxes | 25 | — | — | Dragon Pit roster (egg) |
| Crimson | 25 | 3★ | — | Dragon Pit roster |
| Venator | 25 | 1★ | — | Dragon Pit roster |

---

## Skill Synergies for Updated Lineup

### Army 1: Vhagar / Jagadrix / Bevlorin
- **Vhagar** → Warrior's Resilience (self -8% dmg, +16% Tact to Jagadrix)
- **Jagadrix** → Cunning Whispers (Fire burst + Inst/Init debuff)
- **Bevlorin** → Nature's Reckoning (Phys + Fire DoT), Fire Ward on Vhagar

### Army 2: Vesper / Tashix / Antares
- **Vesper** → Sentinel's Wit (+16% Tact self, +20 Init/Inst to Tashix), Eventide Strike
- **Tashix** → Shimmering Mirage (Fire nukes: 605/590/771), Enervate on enemy Sentinel
- **Antares** → Relentless Pursuit (Fire spread), Silent Shade (Tact multi-hit)

### Army 3: Arrax / Nyrena / Shimmer
- **Arrax** → Sudden Strike (Phys AoE + Weakened on even rounds, scales with Bleed), Warrior's Resilience chain
- **Nyrena** → Undermine (Tact dmg + Phys dmg taken debuff), Ensare (Inst/Init debuff), Breath of Fire (Fire spread, Panic synergy)
- **Shimmer** → Unbreakable Loyalty (Tact +50%, -18% Str, -Initiative debuff), Crushing Force (+9% Phys/Tact), Warrior's Resilience sharing

### Army 4: Solstryker / Rhysarion / Velar
- **Solstryker** → Unknown (Champion class)
- **Rhysarion** → Unknown (Champion class, Lv 24-27)
- **Velar** → Unknown (Champion class, Lv 23 1★)

### Army 5: Dawnseeker / Seasmoke / Feskar
- **Dawnseeker** → Radiant Wings (Command, details unknown)
- **Seasmoke** → Unknown
- **Feskar** → Unknown (Warrior class, 1★ L28)

---

## Notes
- Skill details from Dragon Pit "Summary" tabs — confirm in actual combat
- Star ratings affect skill upgrades (6+ stars unlocks enhanced effects, 10★ max)
- Instinct stat is critical for: Shimmer, Nyrena, Arulix, Arrax, Shadowsong scaling
- Vesper's Sentinel's Wit sharing +20 Init/Inst to Tashix is a key Army 2 mechanic
- Vhagar's Warrior's Resilience sharing +16% Tact to Jagadrix is a key Army 1 mechanic
- Arrax → Shimmer → Nyrena chain: Warrior's Resilience (+16% Tact) → Crushing Force (+9% Phys/Tact) → Undermine/Breath of Fire synergy
- Shadowsong (Hunter) adds Ensare (Round 1 Inst/Init debuff) + Breath of Fire (Rounds 2,5,8 Fire spread, Panic 1.5x)
- Shadowsong stats: Lv25, 3★, Hunter Whelp, Attack 98.6, Defense 87.0, Speed 115.2, Flight 88.3, Class Average, Energy 250/100
- Shadowsong resources: 3,864.0 Food, 13,540 Gold, Player Lv 25, Ensare L1→L2 cost: 100 Purple + 100 Green + 50 Gold
- Daemoros (Lv29 2★ Warrior) adds Shadowflame (odd rounds Phys + Burn)
- Vaeldra (Lv29 3★ Warrior) adds Lure (Taunt + odd-round Phys AoE) + Dragon's Valor (Start -5% dmg/+8.5% Str)# New Dragon Skills — From IMG_2922-2933 (Dragon Pit, 2026-06-19)

---

## Shadowrend (Warrior Whelp, Lv 29, 3★) — Eclipse Fervor (Command)

**Eclipse Fervor (Active)**
- **Base (per round):** 25% chance to apply **Panic** to 1 adjacent enemy (+20% dmg taken, 2 rounds)
- **Rounds 4, 7, 9, 10:** Deal **Physical Damage** to 2 adjacent enemies (DR: **+80%**)
  - Scales with **Strength**, mitigated by **Instinct**
- **At 10★:** Round 9: Deal **Physical + Tactical** to 3 enemies any lane (DR: **+100%** each)
  - No longer restricted to adjacent

**Shadowrend Stats (Lv 29, 3★, Warrior Whelp)**
- **Attack:** 88.6
- **Defense:** 88.6
- **Speed:** 64.0
- **Flight:** 64.0
- **Class:** Slow
- **Energy:** 100 / 100
- **Level:** 29, 3★
- **Class:** Warrior Whelp
- **XP:** 55,577 / 119,717
- **Command:** Eclipse Fervor

---

## Thunderstrike (Warrior Whelp, Lv 29, 3★) — Tail Whip (Command)

**Tail Whip (Active)**
- **Base (< 6★):** Odd rounds (1,3,5,5,5,7,9): Phys DMG to 1 same-lane enemy (DR: **+100%**)
  - Scales with **Strength**, mitigated by **Instinct**
- **At 6★:** Even rounds (2,4,6,8): Phys DMG to 1 same-lane enemy (DR: **+50%**) + 25% chance **Bleed** 2r (+20%)
  - Scales with **Strength**, mitigated by **Instinct**

**Thunderstrike Stats (Lv 29, 3★, Warrior Whelp)**
- **Power/Attack:** 91.0
- **Defense/Health:** 67.7
- **Speed/Accuracy:** 60.3
- **Evasion/Special:** 81.2
- **Rating:** Average
- **Energy:** 100 / 100
- **Level:** 29, 3★
- **Class:** Warrior Whelp
- **Class Progress:** 27 / 100
- **XP:** 38,139 / 119,717
- **Command:** Tail Whip

---

## Solstryker (Champion, Lv 30, 3★) — Unknown
- **Stats:** Lv 30, 3★, Champion
- **Army Assignment:** Army 4 Van

---

## Arulix (Champion Whelp, Lv 29, 3★) — Gleaming Spiral

**Gleaming Spiral (Active)**
- **Every round:** 25% chance to reduce Fire Damage dealt by *all enemies that deal Fire damage* by 15% for 1 round
- **On rounds 1, 2, 3, 5, 8:** Deals **Tactical Damage** to *all enemies that deal Physical Damage* (does not affect Basic-Attack-only enemies), with **+45% Damage Rate**
- **At 6+ Stars:** On rounds 1, 2, 3, 5, 8: Deals **Physical Damage** to 2 enemies in any combat lane, **+20% Damage Rate**. Boosted 1.5x (+30%) on round 5, 2x (+40%) on round 8

---

## Battle Rush (Habit) — Shadowrend/Thunderstrike

**Battle Rush (Habit — Passive)**
- **Trigger:** Start of Round 1
- **Effect:** Increase your unit's **Initiative** by **+25%** (enhanced by Strength), reduce **Instinct** of 2 adjacent enemies by **-15%** (enhanced by Strength). Both effects last 3 rounds.
- **Upgrade Table:**
  | Level | Initiative | Instinct | Power |
  |-------|------------|----------|-------|
  | 1 (current) | +25% | -15% | 250 |
  |
  |
  | 2 | +30% | -18% | 550  |
  | 3 | +35% | -21% | 900  |
  | 4 | +42.5% | -25.5% | 1,300 |
  | 5 | +50% | -30% | 1,800 |
- **Upgrade Cost (L1→L2):** 100 Purple + 100 Green + 176 Gold

---

## Dragon's Valor (Habit) — Vaeldra/Thunderstrike/Shadowrend

**Dragon's Valor (Habit — Passive)**
- **Trigger:** Start of Combat
- **Effect:** Reduce Damage Received by **-5%**, increase **Strength** by **+8.5%**
- **Upgrade Table:**
  | Level | Damage Received | Strength | Power |
  |-------|-----------------|----------|-------|
  | 1 (current) | -5% | 8.5% | 340  |
  | 2 | -6% | 10.2% | 740  |
  | 3 | -7% | 11.9% | 1,200 |
  | 4 | -8.5% | 14.45% | 1,800 |
  | 5 | -10% | 17% | 2,500 |
- **Upgrade Cost (L1→L2):** 100 Purple + 100 Green + 176 Gold

---

## Updated Dragon Roster (from IMG_2922-2933)

| Dragon | Level | Stars | Class | Army Assignment |
|--------|-------|-------|-------|-----------------|
| **Shadowrend** | 29 | 3★ | Warrior | Army 3 Vanguard |
| **Thunderstrike** | 29 | 3★ | Warrior | Army 3 Left |
| **Daemoros** | 29 | 2★ | Warrior | Army 3 Left (alt) |
| **Vaeldra** | 29 | 3★ | Warrior | Army 3 Right |
| **Solstryker** | 30 | 3★ | Champion | Army 4 Van |
| **Arulix** | 29 | 3★ | Champion | — |
| **Shadowrend** | 29 | 4★ | Warrior | Dragon Pit roster |
| **Thunderstrike** | 29 | 4★ | Warrior | Dragon Pit roster |
| **Daemoros** | 29 | 2★ | Warrior | Dragon Pit roster, Lv29 2★ |
| **Vaeldra** | 29 | 3★ | Warrior | Dragon Pit roster, Lv29 3★ |
| **Arulix** | 29 | 3★ | Champion | Dragon Pit roster |
| **Caraxes** | 25 | 3★ | — | Dragon Pit roster |
| **Kalspire** | 25 | 3★ | — | Dragon Pit roster |
| **Crimson** | 25 | 3★ | — | Dragon Pit roster |
| **Venator** | 25 | 1★ | — | Dragon Pit roster |
| **Kalspire** | 25 | — | — | Dragon Pit roster (egg) |
| **Caraxes** | 25 | — | — | Dragon Pit roster (egg) |
| **Crimson** | 25 | 3★ | — | Dragon Pit roster |
| **Venator** | 25 | 1★ | — | Dragon Pit roster |

---

## Updated Army Compositions

### Army 3: Shadowrend / Thunderstrike / Vaeldra (All Warriors)
| Position | Dragon | Level/Stars | Class | Key Ability |
|----------|--------|-------------|-------|-------------|
| **Vanguard** | **Shadowrend** | 29, 3★ | Warrior | Eclipse Fervor (Panic + Phys) |
| **Left** | **Thunderstrike** | 29, 3★ | Warrior | Tail Whip (odd/even Phys + Bleed) |
| **Right** | **Vaeldra** | 29, 3★ | Warrior | Lure (Taunt + Phys) + Dragon's Valor |

**Synergy:**
- All 3 are **Warriors** — share Physical damage focus, Strength scaling
- **Vaeldra Lure** → 25% Taunt 3 enemies + odd-round Phys AoE
- **Shadowrend Eclipse Fervor** → Panic debuff + R4/7/9/10 Phys bursts
- **Thunderstrike Tail Whip** → Odd-round Phys (100%) → At 6★ Even-round Phys + Bleed
- **Battle Rush** (habit) → Start R1: +Init/ -Enemy Inst
- **Dragon's Valor** (habit) → Start: -5% dmg taken, +8.5% Str

### Army 4: Solstryker / Rhysarion / Velar
| Position | Dragon | Level/Stars | Class |
|----------|--------|-------------|-------|
| Van | Solstryker | 30, 3★ | Champion |
| Left | Rhysarion | 24-27, 2★ | Champion |
| Right | Velar | 23, 1★ | Champion |

### Army 5: Shadowsong / Seasmoke / Feskar
| Position | Dragon | Level/Stars | Class |
|----------|--------|-------------|-------|
| Van | Shadowsong | 28, 2★ | Hunter |
| Left | Seasmoke | 25, 2★ | — |
| Right | Feskar | 28, 1★ | Warrior |

---

## Updated Notes

- **Shadowrend** (Lv29 3★ Warrior) — Army 3 Van — Eclipse Fervor (Panic + Phys, R4/7/9/10)
- **Thunderstrike** (Lv29 3★ Warrior) — Army 3 Left — Tail Whip (odd/even Phys, Bleed at 6★)
- **Vaeldra** (Lv29 3★ Warrior) — Army 3 Right — Lure (Taunt + odd Phys) + Dragon's Valor
- **Daemoros** (Lv29 2★ Warrior) — Shadowflame (odd rounds Phys + Burn)
- **Vaeldra** (Lv29 3★ Warrior) — Lure + Dragon's Valor
- **Arulix** (Lv29 3★ Champion) — Gleaming Spiral
- **Solstryker** (Lv30 3★ Champion) — Army 4 Van
- **Army 3** now all Warriors — Physical damage focus, Taunt control, Panic synergy# New Dragon Skills — From IMG_2922-2944 (Dragon Pit, 2026-06-19 to 2026-06-20)

---

## Solstryker (Champion, Lv 30, 3★) — Steady Erosion (Habit) + Tactical Onslaught (Command)

### Steady Erosion (Habit — Passive)
- **Trigger:** Start of each round
- **Effect:** Add 1 Stack of **Steady Erosion** (Max 10 Stacks) to all Enemies
  - Each Stack: Reduces target's **Strength** by **-2%** (enhanced by **Strength**) until end of combat
- **Upgrade Table:**
  | Level | Strength per Stack | Power |
  |-------|-------------------|-------|
  | 1 (current) | -2% | 250 |
  | 2 | -2.4% | 550 |
  | 3 | -2.8% | 900 |
  | 4 | -3.4% | 1,300 |
  | 5 | -4% | 1,800 |
- **Upgrade Cost (L1→L2):** 100 Blue + 100 Green + 160 Gold

### Tactical Onslaught (Command — Active)
- **Trigger:** After every Basic Attack
- **Odd Rounds:** Phys DMG to Basic Attack target (DR: **+30%**) + 20% chance reduce target's Phys DMG Dealt by **-12%** (2r)
- **Even Rounds:** Tact DMG to 3 enemies any lane (DR: **+12.5%**), **2x** vs **Vulnerable**
- **Damage Values:** 4,367 Phys / 11,250 Tact

### Solstryker Stats (Lv 30, 3★, Champion Whelp)
- **Level:** 30, 3★
- **Class:** Champion Whelp
- **Class Progress:** 13 / 100
- **XP:** 117,120 / 132,385
- **Stars:** 3★
- **Stats:** Power 83.6, Armor 89.8, Acc 61.5, Speed 72.6, Rating Average, Energy 95/100
- **Stats (alternate):** Head 83.6, Body 89.8, Eye 61.5, Wing 72.6, Flag Average, Lightning 95/100
- **Abilities:** 1 unlocked (L1), 4 locked
- **Command:** Tactical Onslaught + Steady Erosion
- **XP:** 117,120 / 132,385
- **Resources:** 4,367 Food / 11,250 Gold

---

## Arulix (Champion Whelp, Lv 29, 3★) — Gleaming Spiral + Hypnotic Helix

### Gleaming Spiral (Active)
- **Every round:** 25% chance to reduce Fire Damage dealt by *all enemies that deal Fire damage* by 15% for 1 round
- **Rounds 1, 2, 3, 5, 8:** Tactical DMG to enemies that deal Physical DMG (excluding Basic Attacks), DR **+45%**
- **At 6+ Stars:** Rounds 1, 2, 3, 5, 8: Phys DMG to 2 enemies any lane (DR **+20%**), boosted 1.5x (+30%) R5, 2x (+40%) R8
- **Fire DMG Dealt Modifier (base):** Reduce target's Fire DMG Dealt by **-15%**
- **Phys DMG (At 6★):** DR **+20%**, scales with Str vs Inst
- **Tact DMG (At 6★):** DR **+20%**, scales with Inst vs Int

### Hypnotic Helix (Habit — Passive)
- **Overwhelm Effect:** Prevents target from using Active Commands and Habits on their turn
- **Trigger:** Rounds 2, 4, 7, 9: **13%** chance to **Stagger** 1 enemy any lane (2r), prioritizes Phys DMG dealers
- **Stagger Effect:** Prevents Active Commands and Basic Attacks
- **Upgrade Table:**
  | Level | Stagger Chance | Power |
  |-------|---------------|-------|
  | 1 (current) | 12.5% | 250 |
  | 2 | 15% | 550 |
  | 3 | 17.5% | 900 |
  | 4 | 21.3% | 1,300 |
  | 5 | 25% | 1,800 |
- **Upgrade Cost (L1→L2):** 100 Blue + 100 Green + 160 Gold

### Arulix Stats (Lv 29, 3★, Champion Whelp)
- **Head/Attack:** 115.2
- **Rings/Defense:** 108.8
- **Eye/Speed:** 87.0
- **Wing/Flight:** 94.7
- **Flag/Tier:** Average
- **Lightning/Stamina:** 100 / 100
- **Level Progress:** 81 / 100
- **Level:** 29, 3★
- **Class:** Champion Whelp
- **HP:** 49,725 / 119,717
- **Resources:** 4,266.0 Food / 10,850 Gold

---

## Solstryker (Champion, Lv 30, 3★) — Steady Erosion + Tactical Onslaught

### Steady Erosion (Habit — Passive)
- **Trigger:** Start of each round
- **Effect:** Add 1 Stack of **Steady Erosion** (Max 10) to all Enemies
  - Each Stack: Reduces target's **Strength** by **-2%** (enhanced by **Strength**) until end of combat
- **Upgrade Table:**
  | Level | Strength per Stack | Power |
  |-------|-------------------|-------|
  | 1 | -2% | 250 |
  | 2 | -2.4% | 550 |
  | 3 | -2.8% | 900 |
  | 4 | -3.4% | 1,300 |
  | 5 | -4% | 1,800 |
- **Upgrade Cost (L1→L2):** 100 Blue + 100 Green + 160 Gold

### Tactical Onslaught (Command — Active)
- **Trigger:** After every Basic Attack
- **Odd Rounds:** Phys DMG to Basic Attack target (DR **+30%**) + 20% chance reduce target's Phys DMG Dealt by **-12%** (2r)
- **Even Rounds:** Tact DMG to 3 enemies any lane (DR **+12.5%**), **2x** vs **Vulnerable**
- **Damage Values:** 4,367 Phys / 11,250 Tact

### Solstryker Stats (Lv 30, 3★, Champion Whelp)
- **Level:** 30, 3★
- **Class:** Champion Whelp
- **Class Progress:** 13 / 100
- **XP:** 117,120 / 132,385
- **Stars:** 3★
- **Stats:** Power 83.6, Armor 89.8, Acc 61.5, Speed 72.6, Rating Average, Energy 95/100
- **Stats (alt):** Head 83.6, Body 89.8, Eye 61.5, Wing 72.6, Flag Average, Lightning 95/100
- **Abilities:** 1 unlocked (L1), 4 locked
- **Command:** Tactical Onslaught + Steady Erosion
- **XP:** 117,120 / 132,385
- **Resources:** 4,367 Food / 11,250 Gold

---

## Arulix (Champion Whelp, Lv 29, 3★) — Gleaming Spiral + Hypnotic Helix

### Gleaming Spiral (Active)
- **Every round:** 25% chance to reduce Fire DMG dealt by *all enemies that deal Fire DMG* by 15% for 1 round
- **Rounds 1, 2, 3, 5, 8:** Tactical DMG to enemies that deal Phys DMG (excl Basic Attacks), DR **+45%**
- **At 6+ Stars:** Rounds 1,2,3,5,8: Phys DMG to 2 enemies any lane (DR **+20%**), boosted 1.5x (+30%) R5, 2x (+40%) R8
- **Fire DMG Dealt Modifier (base):** Reduce target's Fire DMG Dealt by **-15%**
- **Phys DMG (At 6★):** DR **+20%**, scales Str vs Inst
- **Tact DMG (At 6★):** DR **+20%**, scales Inst vs Int

### Hypnotic Helix (Habit — Passive)
- **Overwhelm Effect:** Prevents target from using Active Commands and Habits on their turn
- **Trigger:** Rounds 2, 4, 7, 9: **12.5%** chance to **Stagger** 1 enemy any lane (2r), prioritizes Phys DMG dealers
- **Stagger Effect:** Prevents Active Commands and Basic Attacks
- **Upgrade Table:**
  | Level | Stagger Chance | Power |
  |-------|---------------|-------|
  | 1 (current) | 12.5% | 250 |
  | 2 | 15% | 550 |
  | 3 | 17.5% | 900 |
  | 4 | 21.3% | 1,300 |
  | 5 | 25% | 1,800 |
- **Upgrade Cost (L1→L2):** 100 Blue + 100 Green + 160 Gold

### Arulix Stats (Lv 29, 3★, Champion Whelp)
- **Head/Attack:** 115.2
- **Rings/Defense:** 108.8
- **Eye/Speed:** 87.0
- **Wing/Flight:** 94.7
- **Flag/Tier:** Average
- **Lightning/Stamina:** 100 / 100
- **Level Progress:** 81 / 100
- **Level:** 29, 3★
- **Class:** Champion Whelp
- **HP:** 49,725 / 119,717
- **Resources:** 4,266.0 Food / 10,850 Gold

---

## Daemoros (Warrior Hatchling, Lv 29, 2★) — Shadowflame

### Shadowflame (Active)
- **Trigger:** Odd-numbered rounds (1, 3, 5, 7, 9...)
- **Effect:** Phys DMG to 1 adjacent enemy (DR **+125%**) + 20% chance **Burn** 2r (+20%)
  - Scales with **Strength**, mitigated by **Instinct**
- **Daemoros Stats (Lv 29, 2★, Warrior Hatchling):**
  - **Strength:** 4,166.0
  - **Health:** 12,950
  - **HP:** 12,020 / 108,261
  - **Ability Level:** 12 / 40
  - **Level:** 29, 2★

---

## Vaeldra (Warrior Whelp, Lv 29, 3★) — Lure + Dragon's Valor

### Lure (Active)
- **Taunt:** Each Round: 25% chance to **Taunt** 3 enemies any lane (2r)
- **Damage:** Odd rounds: Phys DMG to 2 adjacent (DR **+45%**)
  - Scales with **Strength**, mitigated by **Instinct**

### Dragon's Valor (Habit — Passive)
- **Trigger:** Start of Combat
- **Effect:** Reduce DMG Received **-5%**, increase **Strength** **+8.5%**
- **Upgrade Table:**
  | Level | DMG Received | Strength | Power |
  |-------|--------------|----------|-------|
  | 1 | -5% | 8.5% | 340 |
  | 2 | -6% | 10.2% | 740 |
  | 3 | -7% | 11.9% | 1,200 |
  | 4 | -8.5% | 14.45% | 1,800 |
  | 5 | -10% | 17% | 2,500 |
- **Upgrade Cost (L1→L2):** 100 Purple + 100 Green + 176 Gold

### Vaeldra Stats (Lv 29, 3★, Warrior Whelp)
- **HP:** 32,614 / 97,901
- **Level Progress:** 61 / 100
- **Stars:** 3★
- **Class:** Warrior Whelp
- **Stats:** Head 115.2, Rings 108.8, Eye 87.0, Wing 94.7, Flag Average, Lightning 100/100
- **Level Progress:** 61 / 100

---

## Battle Rush (Habit — Passive)
- **Trigger:** Start of Round 1
- **Effect:** +25% Initiative (enhanced by Str), -15% Inst to 2 adjacent enemies (enhanced by Str), both 3 rounds
- **Upgrade Table:**
  | Level | Initiative | Instinct | Power |
  |-------|------------|----------|-------|
  | 1 | +25% | -15% | 250 |
  | 2 | +30% | -18% | 550 |
  | 3 | +35% | -21% | 900 |
  | 4 | +42.5% | -25.5% | 1,300 |
  | 5 | +50% | -30% | 1,800 |
- **Upgrade Cost (L1→L2):** 100 Blue + 100 Green + 160 Gold

---

## Dragon's Valor (Habit — Passive)
- **Trigger:** Start of Combat
- **Effect:** Reduce DMG Received **-5%**, increase **Strength** **+8.5%**
- **Upgrade Table:**
  | Level | DMG Received | Strength | Power |
  |-------|--------------|----------|-------|
  | 1 | -5% | 8.5% | 340 |
  | 2 | -6% | 10.2% | 740 |
  | 3 | -7% | 11.9% | 1,200 |
  | 4 | -8.5% | 14.45% | 1,800 |
  | 5 | -10% | 17% | 2,500 |
- **Upgrade Cost (L1→L2):** 100 Purple + 100 Green + 176 Gold

---

## Updated Dragon Roster (from IMG_2929, IMG_2943)

| Dragon | Level | Stars | Class | Army Assignment |
|--------|-------|-------|-------|-----------------|
| **Solstryker** | 30 | 3★ | Champion | Army 4 Van |
| **Arulix** | 29 | 3★ | Champion | — |
| **Shadowrend** | 29 | 2★ | Warrior | Army 3 Van |
| **Thunderstrike** | 29 | 3★ | Warrior | Army 3 Left |
| **Daemoros** | 28 | 1★ | Warrior | Army 3 Left (alt) |
| **Vaeldra** | 27 | 1★ | Warrior | Army 3 Right |
| **Shadowsong** | 25 | 3★ | Hunter | Army 5 Van |
| **Shadowrend** | 29 | 4★ | Warrior | Dragon Pit roster |
| **Thunderstrike** | 29 | 4★ | Warrior | Dragon Pit roster |
| **Daemoros** | 28 | 1★ | Warrior | Dragon Pit roster, Lv28 1★ |
| **Vaeldra** | 27 | 1★ | Warrior | Dragon Pit roster, Lv27 1★ |
| **Arulix** | 29 | 3★ | Champion | Dragon Pit roster |
| **Caraxes** | 25 | 3★ | — | Dragon Pit roster |
| **Kalspire** | 25 | 3★ | — | Dragon Pit roster |
| **Crimson** | 25 | 3★ | — | Dragon Pit roster |
| **Venator** | 25 | 1★ | — | Dragon Pit roster |
| **Kalspire** | 25 | — | — | Egg |
| **Caraxes** | 25 | — | — | Egg |
| **Crimson** | 25 | 3★ | — | Dragon Pit roster |
| **Venator** | 25 | 1★ | — | Dragon Pit roster |

---

## Army 3: Solstryker / Arulix / ? (Champion Core)

| Position | Dragon | Level/Stars | Class | Key Ability |
|----------|--------|-------------|-------|-------------|
| Van | Solstryker | 30, 3★ | Champion | Tactical Onslaught + Steady Erosion |
| Left | Arulix | 29, 3★ | Champion | Gleaming Spiral + Hypnotic Helix |
| Right | ? | ? | ? | ? |

**Champion Core Synergy:**
- **Arulix** → Gleaming Spiral (Fire reduction, Tactical vs Phys, 6★ Phys), Hypnotic Helix (Stagger + Overwhelm)
- **Solstryker** → Tactical Onslaught (odd/even Phys/Tact, Vulnerable 2x), Steady Erosion (Str debuff stacks)
- **Champion Core:** High Tactical/Physical synergy, stacking debuffs, turn manipulation

---

## Updated Army Compositions

### Army 1: Vhagar / Jagadrix / Bevlorin (Validated VICTORY)
### Army 2: Vesper / Tashix / Antares (Validated VICTORY)
### Army 3: Arrax / Daemoros / Vaeldra (Warrior Core)
### Army 4: Solstryker / Rhysarion / Velar (Champion Core)
### Army 5: Shadowsong / Seasmoke / Feskar

---

## Resource Requirements Summary

| Upgrade | Blue | Green | Gold |
|---------|------|-------|------|
| Habit L1→L2 (Steady Erosion, Battle Rush) | 100 | 100 | 160 |
| Habit L1→L2 (Dragon's Valor) | 100 Purple | 100 Green | 176 |
| Habit L1→L2 (Battle Rush, Hypnotic Helix) | 100 Blue | 100 Green | 160 |
| Habit L1→L2 (Ensare, Shadowflame) | 100 Purple | 100 Green | 50/176 Gold |
| Ensare L1→L2 | 100 Purple + 100 Green + 50 Gold |
| Shadowflame L1→L2 | (unknown) |

---

## Notes
- **Solstryker** (Lv30 3★ Champion) — Army 4 Van — Tactical Onslaught (odd/even Phys/Tact, Vulnerable 2x) + Steady Erosion (Str stacks)
- **Arulix** (Lv29 3★ Champion) — Gleaming Spiral (Fire reduction, Tactical vs Phys, 6★ Phys), Hypnotic Helix (Stagger + Overwhelm)
- **Solstryker** (Lv30 3★) — Army 4 Van — Tactical Onslaught + Steady Erosion
- **Arulix** (Lv29 3★ Champion) — Gleaming Spiral + Hypnotic Helix
- **Solstryker** Steady Erosion: Max 10 stacks of -2% Str/enemy/round
- **Solstryker** Tactical Onslaught: Odd rounds Phys + Phys dmg reduction; Even rounds Tact 3-target, 2x vs Vulnerable
- **Arulix** Gleaming Spiral: Fire reduction, Tactical vs Phys, 6★ Phys to 2 any lane
- **Arulix** Hypnotic Helix: Stagger (disable commands/attacks) + Overwhelm (disable active/habit), 12.5%→25% chance
- **Champion Core** (Army 4): Solstryker + Arulix — high Tactical/Physical, turn manipulation, stacking debuffs# Additional Dragon Skills — From IMG_2943-2952 (Dragon Pit, 2026-06-19 to 2026-06-20)

---

## Arrax (Warrior Whelp, Lv 31, 3★) — Sudden Strike (Command) + Headlong Into Danger (Habit)

### Sudden Strike (Active)
- **Trigger:** Rounds 2, 4, 5, 6, 8
- **Debuff:** 25% chance **Weakened** to 1 adjacent enemy (-10% dmg dealt). **50% chance** if target has **Bleed**
- **Damage:** Rounds 2, 4, 5, 6, 8: Phys DMG to 2 adjacent enemies (DR: **+40%**)
  - Scales with **Strength**, mitigated by **Instinct**

### Headlong Into Danger (Habit — Passive)
- **Trigger:** Start of Round 4
- **Buffs (4 rounds):** +25% Strength, +10% Initiative, +25% Physical DMG Dealt
- **Debuffs (4 rounds):** +10% Physical DMG Received, -40% Instinct
- **Upgrade Table:**
  | Level | DMG Received | Instinct | Power |
  |-------|--------------|----------|-------|
  | 1 | 10% | -40% | 250 |
  | 2 | 8% | -32% | 550 |
  | 3 | 6% | -24% | 900 |
  | 4 | 3% | -12% | 1,300 |
  | 5 | 0% | 0% | 1,800 |
- **Upgrade Cost (L1→L2):** 100 Blue + 100 Green + 176 Gold

### Arrax Stats (Lv 31, 3★, Warrior Whelp)
- **Attack/Head:** 84.9 / 83.6 / 88.6 / 86.1 / 115.2 (varies by screenshot)
- **Defense/Rings:** 86.1 / 88.6 / 82.5 / 108.8
- **Speed/Eye:** 66.4 / 64.0 / 87.0 / 59.0
- **Flight/Wing:** 76.3 / 64.0 / 94.7 / 67.7
- **Class:** Slow / Average
- **Energy:** 96/100 / 100/100
- **Level:** 31, 3★
- **Class:** Warrior Whelp
- **HP:** 129,999 / 146,394 (XP)
- **Command:** Sudden Strike
- **Class Progress:** 18 / 100
- **Resources:** 4,467 Food / 11,450 Gold (varies)

---

## Daemoros (Warrior Hatchling, Lv 28, 1★ → Lv 29, 2★) — Shadowflame

### Shadowflame (Active)
- **Trigger:** Odd-numbered rounds (1, 3, 5, 7, 9...)
- **Effect:** Phys DMG to 1 adjacent enemy (DR: **+125%**) + 20% chance **Burn** 2r (+20%)
  - Scales with **Strength**, mitigated by **Instinct**
- **Daemoros Stats (Lv 29, 2★, Warrior Hatchling):**
  - **Strength:** 4,166.0
  - **Health:** 12,950
  - **HP:** 12,020 / 108,261
  - **Ability Level:** 12 / 40
  - **Level:** 29, 2★

---

## Vaeldra (Warrior Whelp, Lv 27, 1★ → Lv 29, 3★) — Lure + Dragon's Valor

### Lure (Active)
- **Taunt:** Each Round: 25% chance to **Taunt** 3 enemies any lane (2r)
- **Damage:** Odd rounds: Phys DMG to 2 adjacent (DR **+45%**)
  - Scales with **Strength**, mitigated by **Instinct**

### Dragon's Valor (Habit — Passive)
- **Trigger:** Start of Combat
- **Effect:** Reduce DMG Received **-5%**, increase **Strength** **+8.5%**
- **Upgrade Table:**
  | Level | DMG Received | Strength | Power |
  |-------|--------------|----------|-------|
  | 1 | -5% | 8.5% | 340 |
  | 2 | -6% | 10.2% | 740 |
  | 3 | -7% | 11.9% | 1,200 |
  | 4 | -8.5% | 14.45% | 1,800 |
  | 5 | -10% | 17% | 2,500 |
- **Upgrade Cost (L1→L2):** 100 Purple + 100 Green + 176 Gold

### Vaeldra Stats (Lv 29, 3★, Warrior Whelp)
- **HP:** 32,614 / 97,901
- **Level Progress:** 61 / 100
- **Stars:** 3★
- **Class:** Warrior Whelp
- **Stats:** Head 115.2, Rings 108.8, Eye 87.0, Wing 94.7, Flag Average, Lightning 100/100
- **Level Progress:** 61 / 100

---

## Vhagar (Warrior Hatchling, Lv 30, 4★) — Fiery Bonds (Command)

### Fiery Bonds (Active)
- **Taunt:** Each Round: 25% chance to **Taunt 3** enemies any lane (2r). **50%** if target has **Burn**
- **Damage:** Even rounds: Phys DMG to 1 adjacent (DR **+120%**)
  - Scales with **Strength**, mitigated by **Instinct**

### Vhagar Stats (Lv 30, 4★, Warrior Hatchling)
- **HP:** 32,614 / 97,901 (Lv 30)
- **Level Progress:** 0 / 40
- **Command:** Fiery Bonds
- **Attack/Head:** 136.7
- **Defense/Rings:** 143.5
- **Speed/Eye:** 99.4
- **Flight/Wing:** 114.1
- **Class:** Slow

---

## Battle Rush (Habit — Passive)
- **Trigger:** Start of Round 1
- **Effect:** +25% Initiative (enhanced by Str), -15% Inst to 2 adjacent enemies (enhanced by Str). Both 3 rounds.
- **Upgrade Table:**
  | Level | Initiative | Instinct | Power |
  |-------|------------|----------|-------|
  | 1 | +25% | -15% | 250 |
  | 2 | +30% | -18% | 550 |
  | 3 | +35% | -21% | 900 |
  | 4 | +42.5% | -25.5% | 1,300 |
  | 5 | +50% | -30% | 1,800 |
- **Upgrade Cost (L1→L2):** 100 Blue + 100 Green + 160 Gold

---

## Dragon's Valor (Habit — Passive)
- **Trigger:** Start of Combat
- **Effect:** Reduce DMG Received **-5%**, increase **Strength** **+8.5%**
- **Upgrade Table:**
  | Level | DMG Received | Strength | Power |
  |-------|--------------|----------|-------|
  | 1 | -5% | 8.5% | 340 |
  | 2 | -6% | 10.2% | 740 |
  | 3 | -7% | 11.9% | 1,200 |
  | 4 | -8.5% | 14.45% | 1,800 |
  | 5 | -10% | 17% | 2,500 |
- **Upgrade Cost (L1→L2):** 100 Purple + 100 Green + 176 Gold

---

## Updated Dragon Roster (from IMG_2929, IMG_2943-2952)

| Dragon | Level | Stars | Class | Army Assignment |
|--------|-------|-------|-------|-----------------|
| **Arrax** | 31 | 3★ | Warrior | Army 3 Van |
| **Vhagar** | 30 | 4★ | Warrior | Army 3 Right |
| **Solstryker** | 30 | 3★ | Champion | Army 4 Van |
| **Arulix** | 29 | 3★ | Champion | — |
| **Shadowrend** | 29 | 2★ | Warrior | Warrior | Army 3 Van |
| **Thunderstrike** | 29 | 3 | Warrior | Army 3 Left |
| **Daemoros** | 28 | 1 | Warrior | Army 3 Left (alt) |
| **Vaeldra** | 27 | 1 | Warrior | Army 3 Right |
| **Solstryker** | 30 | 3★ | Champion | Army 4 Van |
| **Arulix** | 29 | 3★ | Champion | — |
| **Shadowrend** | 29 | 4★ | Warrior | Dragon Pit roster |
| **Thunderstrike** | 29 | 4★ | Warrior | Dragon Pit roster |
| **Daemoros** | 28 | 1★ | Warrior | Dragon Pit roster, Lv28 1★ |
| **Vaeldra** | 27 | 1★ | Warrior | Dragon Pit roster, Lv27 1★ |
| **Arulix** | 29 | 3★ | Champion | Dragon Pit roster |
| **Caraxes** | 25 | 3★ | — | Dragon Pit roster |
| **Kalspire** | 25 | 3★ | — | Dragon Pit roster |
| **Crimson** | 25 | 3★ | — | Dragon Pit roster |
| **Venator** | 25 | 1★ | — | Dragon Pit roster |
| **Kalspire** | 25 | — | — | Egg |
| **Caraxes** | 25 | — | — | Egg |
| **Crimson** | 25 | 3★ | — | Dragon Pit roster |
| **Venator** | 25 | 1★ | — | Dragon Pit roster |

---

## Updated Army Compositions

### Army 3: Arrax / Daemoros / Vaeldra (Warrior Core)
| Position | Dragon | Level/Stars | Class | Key Ability |
|----------|--------|-------------|-------|-------------|
| Van | Arrax | 31, 3 | Warrior | Sudden Strike (Phys AoE), Headlong Into Danger |
| Left | Daemoros | 29, 2 | Warrior | Shadowflame (Odd rounds Phys + Burn) |
| Right | Vaeldra | 29, 3 | Warrior | Lure (Taunt + odd Phys) + Dragon's Valor |

### Army 3 (Alt): Shadowrend / Thunderstrike / Vaeldra
| Position | Dragon | Level/Stars | Class | Key Ability |
|----------|--------|-------------|-------|-------------|
| Van | Shadowrend | 29, 2 | Warrior | Eclipse Fervor (Panic + Phys) |
| Left | Thunderstrike | 29, 3 | Warrior | Tail Whip (Odd/even Phys, Bleed at 6) |
| Right | Vaeldra | 29, 3 | Warrior | Lure (Taunt + odd Phys) + Dragon's Valor |

### Army 4: Solstryker / Arulix / ? (Champion Core)
| Position | Dragon | Level/Stars | Class | Key Ability |
|----------|--------|-------------|-------|-------------|
| Van | Solstryker | 30, 3 | Champion | Tactical Onslaught + Steady Erosion |
| Left | Arulix | 29, 3 | Champion | Gleaming Spiral + Hypnotic Helix |
| Right | ? | ? | ? | ? |

### Army 5: Shadowsong / Seasmoke / Feskar
| Position | Dragon | Level/Stars | Class |
|----------|--------|-------------|-------|
| Van | Shadowsong | 28, 2 | Hunter |
| Left | Seasmoke | 25, 2 | — |
| Right | Feskar | 28, 1 | Warrior |

---

## Skill Synergies

### Army 3 (Warrior Core): Arrax / Daemoros / Vaeldra
| Position | Dragon | Key Synergy |
|----------|--------|-------------|
| Van | Arrax | Sudden Strike (R2/4/5/6/8 Phys AoE + Weakened), Headlong Into Danger (R4: +25% Str/+25% Phys/+10% Init/-40% Inst/-10% Phys taken) |
| Left | Daemoros | Shadowflame (Odd rounds: Phys +125% + 20% Burn 2r) |
| Right | Vaeldra | Lure (25% Taunt 3 enemies, odd rounds Phys +45%), Dragon's Valor (-5% dmg/+8.5% Str) |

### Army 3 (Alt): Shadowrend / Thunderstrike / Vaeldra
| Position | Dragon | Key Synergy |
|----------|--------|-------------|
| Van | Shadowrend | Eclipse Fervor (25% Panic/round, R4/7/9/10 Phys 80%, 10★ Phys+Tact to 3) |
| Left | Thunderstrike | Tail Whip (Odd: Phys 100% 1 target; 6★: Even Phys 50% + 25% Bleed 2r) |
| Right | Vaeldra | Lure + Dragon's Valor (same as above) |

### Army 4 (Champion Core): Solstryker + Arulix
| Position | Dragon | Key Synergy |
|----------|--------|-------------|
| Van | Solstryker | Steady Erosion (-2% Str/stack, max 10 stacks), Tactical Onslaught (Odd: Phys 30% + 12% Phys reduction; Even: Tact 12.5% to 3, 2x vs Vuln) |
| Left | Arulix | Gleaming Spiral (Fire -15%, Tact vs Phys 45%, 6★ Phys to 2 any lane), Hypnotic Helix (12.5% Stagger + Overwhelm) |
| Right | ? | ? |

### Synergy Notes
- **Warrior Core (Army 3):** All Physical damage, odd/even round staggering, Taunt control (Vaeldra), Panic synergy (Shadowrend)
- **Champion Core (Army 4):** High Tactical/Physical, turn manipulation (Stagger/Overwhelm), stacking Str debuffs (Steady Erosion), Vulnerable synergy (Tactical Onslaught 2x vs Vuln)
- **Habits:** Battle Rush (Start R1: +Init/-Inst), Dragon's Valor (-5% dmg/+8.5% Str), Steady Erosion (Str stacks), Battle Rush (+Init/-Inst), Dragon's Valor (-5% dmg/+8.5% Str)

---

## Updated Resource Costs

| Upgrade | Blue | Green | Purple | Gold |
|---------|------|-------|--------|------|
| Habit L1→L2 (Steady Erosion, Battle Rush, Hypnotic Helix) | 100 | 100 | — | 160 |
| Habit L1→L2 (Dragon's Valor) | — | 100 | 100 | 176 |
| Habit L1→L2 (Headlong Into Danger) | 100 | 100 | — | 176 |
| Habit L1→L2 (Dragon's Valor) | 100 Purple | 100 Green | — | 176 |
| Habit L1→L2 (Battle Rush, Hypnotic Helix, Steady Erosion) | 100 Blue | 100 Green | — | 160 |
| Ensare L1→L2 | 100 Purple | 100 Green | — | 50 |
| Headlong Into Danger L1→L2 | 100 Blue | 100 Green | — | 176 |
| Shadowflame L1→L2 | (unknown) | — | — | — |

---

## Notes
- **Arrax** (Lv31 3 Warrior) — Sudden Strike (R2/4/5/6/8 Phys AoE + Weakened), Headlong Into Danger (R4: +25% Str/+25% Phys/+10% Init/-40% Inst/-10% Phys taken)
- **Solstryker** (Lv30 3★ Champion) — Army 4 Van — Tactical Onslaught (odd/even Phys/Tact, Vuln 2x) + Steady Erosion (Str stacks)
- **Arulix** (Lv29 3★ Champion) — Gleaming Spiral (Fire reduction, Tact vs Phys, 6★ Phys AoE), Hypnotic Helix (Stagger/Overwhelm)
- **Solstryker** Steady Erosion: Max 10 stacks of -2% Str/enemy/round
- **Solstryker** Tactical Onslaught: Odd rounds Phys + Phys dmg reduction; Even rounds Tact 3-target, 2x vs Vulnerable
- **Arulix** Gleaming Spiral: Fire reduction, Tact vs Phys, 6★ Phys AoE
- **Arulix** Hypnotic Helix: Stagger (disable commands/attacks) + Overwhelm (disable active/habit)
- **Champion Core** (Army 4): Solstryker + Arulix — high Tactical/Physical, turn manipulation, stacking Str debuffs, Panic synergy with Shadowrend# Velar (Sentinel Hatchling, Lv 31, 3★) — Whirlwind (Command)

### Whirlwind (Active)
- **Recovery:** Rounds 2, 4, 6, 8: Apply **Recovery** to 3 Allies in any lane (Recovery Rate: **+18%**, enhanced by **Initiative**)
- **Tactical Damage:** Deal **Tactical Damage** to target (DR: **+45%**)
  - Scales with attacker's **Instinct**, mitigated by target's **Intelligence**
- **Advantage:** Increase target's Damage Dealt by **+15%**

### Velar Stats (Lv 31, 3★, Sentinel Hatchling)
- **Level:** 31, 3★
- **Class:** Sentinel Hatchling
- **Skill Progress:** 7 / 40
- **Stars:** 0/5 (unfilled)
- **Locked Slots:** 5/5
- **Skill XP:** 71,289 / 146,394
- **Command:** WHIRLWIND
- **Resources:** 4,467 Food / 14,950 Gold

---

## Updated Velar Entry

### Velar (Sentinel Hatchling, Lv 31, 3★) — Whirlwind (Command) + Lure + Dragon's Valor

### Whirlwind (Active)
- **Recovery:** Rounds 2, 4, 6, 8: Apply **Recovery** to 3 Allies in any lane (Recovery Rate: **+18%**, enhanced by **Initiative**)
- **Tactical Damage:** Deal **Tactical Damage** to target (DR: **+45%**)
  - Scales with attacker's **Instinct**, mitigated by target's **Intelligence**
- **Advantage:** Increase target's Damage Dealt by **+15%"

### Lure (Active)
- **Taunt:** Each Round: 25% chance to **Taunt** 3 enemies any lane (2r)
- **Damage:** Odd rounds: Phys DMG to 2 adjacent (DR **+45%**)
  - Scales with **Strength**, mitigated by **Instinct**

### Dragon's Valor (Habit — Passive)
- **Trigger:** Start of Combat
- **Effect:** Reduce DMG Received **-5%**, increase **Strength** **+8.5%**
- **Upgrade Table:**
  | Level | DMG Received | Strength | Power |
  |-------|--------------|----------|-------|
  | 1 | -5% | 8.5% | 340 |
  | 2 | -6% | 10.2% | 740 |
  | 3 | -7% | 11.9% | 1,200 |
  | 4 | -8.5% | 14.45% | 1,800 |
  | 5 | -10% | 17% | 2,500 |
- **Upgrade Cost (L1→L2):** 100 Purple + 100 Green + 176 Gold

### Velar Stats (Lv 31, 3★, Sentinel Hatchling)
- **Level:** 31, 3★
- **Class:** Sentinel Hatchling
- **Skill Progress:** 7 / 40
- **Stars:** 0/5 (unfilled)
- **Locked Slots:** 5/5
- **Skill XP:** 71,289 / 146,394
- **Command:** WHIRLWIND
- **Resources:** 4,467 Food / 14,950 Gold# Dawnsong (Dragon, Lv 32, 3★) — Fire Damage + Recovery (Command)

### Fire Damage (Active — Base)
- **Trigger:** Active command
- **Effect:** Deal **Fire Damage** to target (Damage Rate: **+20%**)
  - Scales with attacker's **Intelligence**, mitigated by target's **Initiative**

### Recovery (Active — Unlocked at 6★)
- **Unlock:** Dragon reaches 6★ rarity
- **Effect:** Restore Troops to target (Recovery Rate: **+60%**, enhanced by **Intelligence**)
  - Scales with **player Level**

### Dawnsong Stats (Lv 32, 3★)
- **Dragon:** Dawnsong
- **Level:** 32
- **Stars:** 3★
- **Commander:** Rhysarion (Champion Whelp, Lv 32)
- **XP:** 113,805 / 161,885
- **Level Progress:** 76 / 100
- **Stars:** 3★ (2 unlocked, 4 locked)
- **Command:** Dawnsong

---

## Updated Dragon Roster (from IMG_2974)

| Dragon | Level | Stars | Class | Commander |
|--------|-------|-------|-------|-----------|
| Bevlorin | 34 | 3★ | — | — |
| Seasmoke | 33 | 3★ | — | — |
| **Rhysarion** | 32 | 3★ | Champion | Dawnsong |
| Nyrena | 32 | 3★ | — | — |
| Velar | 31 | 1★ | Sentinel | — |
| Shimmer | 31 | 3★ | Sentinel | — |
| Arrax | 31 | 3★ | Warrior | — |
| Vhagar | 30 | 3★ | Warrior | — |
| Solstryker | 30 | 3★ | Champion | — |
| Arulix | 29 | 3★ | Champion | — |
| Shadowrend | 29 | 2★ | Warrior | — |
| Thunderstrike | 29 | 3★ | Warrior | — |
| Daemoros | 28 | 1★ | Warrior | — |
| Vaeldra | 27 | 1★ | Warrior | — |
| Shadowsong | 25 | 3★ | Hunter | — |# Vesper (Sentinel Whelp, Lv 34, 2★) — Eventide Strike (Command)

### Eventide Strike (Command — Active)
- **Type:** Tactical Damage (primary)
- **Cooldown/Trigger:** Event-based (Command)
- **Damage Type:** Tactical
- **Scaling:** Scales with **Instinct** (attacker), mitigated by **Intelligence** (target)
- **Additional Effects:** Likely includes debuffs or secondary effects (typical for Sentinel skills)

### Vesper Stats (Lv 34, 2★, Sentinel Whelp)
- **Level:** 34
- **Stars:** 2★
- **Class:** Sentinel Whelp
- **Stats:**
  - **Attack:** 65.2
  - **Defense:** 93.5
  - **Health:** 71.3
  - **Power/Spell:** 89.8
  - **Speed:** Fast
  - **Energy:** 88 / 100
- **Level:** 34
- **Class Progress:** 7 / 100
- **XP:** 120,339 / 197,958
- **Command:** Eventide Strike
- **Stars:** 2★ (1 unlocked skill, 4 locked)
- **Skill Progress:** 7 / 100

### Vesper in Army 2 (Validated)
| Position | Dragon | Level/Stars | Class | Key Ability |
|----------|--------|-------------|-------|-------------|
| Vanguard | **Vesper** | 34, 2★ | Sentinel | Eventide Strike (Tactical, Command) |
| Left | Tashix | 29, 2★ | Hunter | Shimmering Mirage (Fire nukes) |
| Right | Antares | 29, 3★ | Champion | Relentless Pursuit + Silent Shade |

**Army 2 Synergy:**
- **Vesper (Vanguard):** Eventide Strike (Tactical, Command) + Sentinel's Wit chain (+24%→+40% Tact, shares +20 Init/Inst to Tashix)
- **Tashix (Left):** Shimmering Mirage (Fire nukes: 605/590/771), Enervate on enemy Sentinel
- **Antares (Right):** Relentless Pursuit (Fire spread) + Silent Shade (Tact multi-hit)
- **Result:** VICTORY R9 vs Level 8 Food, 77% Shieldbearer HP retained, 0 dragon deaths# Feskar (Champion Hatchling, Lv 34, 3★) — Calculated Assault (Command)

### Calculated Assault (Active)
- **Physical Damage Dealt Modifier:** Reduce target's Physical Damage Dealt by **-12%**
- **Fire Damage (At 6+ Stars):** Deal **Fire Damage** to target (Damage Rate: **+40%**)
  - Scales with attacker's **Intelligence**, mitigated by target's **Initiative**
  - Unlocks at 6★ rarity

### Feskar Stats (Lv 34, 3★, Champion Hatchling)
- **Level:** 34
- **Stars:** 3★
- **Class:** Champion Hatchling
- **Health/Power:** 4,769.0
- **Fire Stat:** 15,950
- **Roster Position:** Army 3 Left (alt)

---

## Updated Feskar / Daemoros / Vaeldra Army 3

| Position | Dragon | Level/Stars | Class | Key Ability |
|----------|--------|-------------|-------|-------------|
| Vanguard | Arrax | 31, 3★ | Warrior | Sudden Strike + Headlong Into Danger |
| Left | **Feskar** | 34, 3★ | Champion | Calculated Assault (-12% Phys dmg, 6★ Fire +40%) |
| Right | Vaeldra | 29, 3★ | Warrior | Lure + Dragon's Valor |

---

## Updated Feskar Entry

### Feskar (Champion Hatchling, Lv 34, 3★) — Calculated Assault (Command)

### Calculated Assault (Active)
- **Physical Damage Dealt Modifier:** Reduce target's Physical Damage Dealt by **-12%**
- **Fire Damage (At 6+ Stars):** Deal **Fire Damage** to target (Damage Rate: **+40%**)
  - Scales with attacker's **Intelligence**, mitigated by target's **Initiative**
  - Unlocks at 6★ rarity

### Feskar Stats (Lv 34, 3★, Champion Hatchling)
- **Level:** 34
- **Stars:** 3★
- **Class:** Champion Hatchling
- **Health/Power:** 4,769.0
- **Fire Stat:** 15,950
- **Roster Position:** Army 3 Left (alt)

---

## Updated Army 3 Compositions

### Army 3 (Warrior Core): Arrax / Feskar / Vaeldra
| Position | Dragon | Level/Stars | Class | Key Ability |
|----------|--------|-------------|-------|-------------|
| Vanguard | Arrax | 31, 3★ | Warrior | Sudden Strike + Headlong Into Danger |
| Left | **Feskar** | 34, 3★ | Champion | Calculated Assault (-12% Phys dmg, 6★ Fire +40%) |
| Right | Vaeldra | 29, 3★ | Warrior | Lure + Dragon's Valor |

### Army 3 (Alt): Shadowrend / Thunderstrike / Vaeldra
| Position | Dragon | Level/Stars | Class | Key Ability |
|----------|--------|-------------|-------|-------------|
| Vanguard | Shadowrend | 29, 2★ | Warrior | Eclipse Fervor (Panic + Phys) |
| Left | Thunderstrike | 29, 3★ | Warrior | Tail Whip (Odd/even Phys, Bleed at 6★) |
| Right | Vaeldra | 29, 3★ | Warrior | Lure + Dragon's Valor |# Antares (Hunter Whelp, Lv 35, 3★) — Relentless Pursuit (Command)

### Relentless Pursuit (Active)
- **Fire Damage:** Deal **Fire Damage** to target (Damage Rate: **+65%**)
  - Scales with attacker's **Intelligence**, mitigated by target's **Initiative**
- **Vulnerable:** Increase target's Damage Received by **+10%**
- **Fire Damage (At 6+ Stars):** Deal **Fire Damage** to target (Damage Rate: **+20%**)

### Antares Stats (Lv 35, 3★, Hunter Whelp)
- **Level:** 35
- **Stars:** 3★
- **Class:** Hunter Whelp
- **Health/HP:** 76,907 / 218,905
- **Level Progress:** 1 / 100
- **Stars:** 3★
- **Class:** Hunter Whelp

---

## Updated Antares Entry

### Antares (Hunter Whelp, Lv 35, 3★) — Relentless Pursuit (Command)

### Relentless Pursuit (Active)
- **Fire Damage:** Deal **Fire Damage** to target (Damage Rate: **+65%**)
  - Scales with attacker's **Intelligence**, mitigated by target's **Initiative**
- **Vulnerable:** Increase target's Damage Received by **+10%**
- **Fire Damage (At 6+ Stars):** Deal **Fire Damage** to target (Damage Rate: **+20%**)

### Antares Stats (Lv 35, 3★, Hunter Whelp)
- **Level:** 35
- **Stars:** 3★
- **Class:** Hunter Whelp
- **Health/HP:** 76,907 / 218,905
- **Level Progress:** 1 / 100
- **Stars:** 3★
- **Class:** Hunter Whelp